...
export function CondominioCombobox({ value, onChange, className }: Props) {
  ...
  const exactMatchExists = condominios.some(
    c => c.nome.toLowerCase() === searchTerm.trim().toLowerCase()
  );
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-card border border-border rounded-xl shadow-lg overflow-hidden">
          ...
          <ul className="max-h-48 overflow-y-auto p-1">
            <li 
              className="px-3 py-2 text-sm cursor-pointer hover:bg-muted rounded-md"
            ...
            {filteredOptions.map(condo => (
              <li 
                key={condo.id}
                className="px-3 py-2 text-sm cursor-pointer hover:bg-muted rounded-md"
                onClick={() => handleSelect(condo.nome)}
              >
                {condo.nome}
              </li>
            ))}
            ...
            {searchTerm.trim() && !exactMatchExists && (
              <li 
                className="px-3 py-2 text-sm cursor-pointer hover:bg-primary/10 text-primary flex items-center gap-2 rounded-md font-medium"
